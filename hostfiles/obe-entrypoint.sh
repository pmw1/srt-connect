...
sleep 2
screen -p 0 -S $NAME -X stuff $"set stream opts 0:pid=1000,threads=6,format=avc,profile=high,level=5.1,aspect-ratio=16:9,intra-refresh=1,bitrate=5000\012"
screen -p 0 -S $NAME -X stuff $"set stream opts 1:pid=1001,bitrate=256,format=aac,aac-profile=aac-lc,aac-encap=latm\012"
screen -p 0 -S $NAME -X stuff $"set muxer opts ts-id=1,program-num=1,pmt-pid=256,ts-type=dvb,cbr=1,ts-muxrate=5500000,pcr-pid=512\012"
screen -p 0 -S $NAME -X stuff $"set stream opts 0:lookahead=5,keyint=30,bframes=1\012"
screen -p 0 -S $NAME -X stuff $"set stream opts 0:bitrate=5000\012"
